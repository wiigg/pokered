...
; after battle and are detected in wToggleableObjectList.)
	...
	toggle_consts_for BILLS_SECRET_GARDEN
	const TOGGLE_BILLS_SECRET_GARDEN_PIKACHU   ; E4

; Append-only Red+ objects are loaded by the supplemental object lookup.
	const TOGGLE_MT_MOON_POKECENTER_WANDERER  ; E5
	const TOGGLE_VERMILION_DOCK_WANDERER       ; E6
	const TOGGLE_POKEMON_TOWER_1F_WANDERER     ; E7
	const TOGGLE_ROUTE_25_WANDERER             ; E8
	const TOGGLE_POKEMON_MANSION_B1F_WANDERER  ; E9
	const TOGGLE_CERULEAN_CAVE_B1F_WANDERER    ; EA
	const TOGGLE_REDS_HOUSE_2F_DIPLOMA          ; EB
	const TOGGLE_ROUTE_23_SHORTS_YOUNGSTER      ; EC
	const TOGGLE_ROUTE_20_VISITING_ARTICUNO     ; ED
	const TOGGLE_ROUTE_10_VISITING_ZAPDOS       ; EE
	const TOGGLE_CINNABAR_VISITING_MOLTRES      ; EF

	toggle_consts_for VERMILION_DOCK
	const TOGGLE_VERMILION_DOCK_GHOST_SAILOR   ; F0

	toggle_consts_for FUCHSIA_CITY
	const TOGGLE_FUCHSIA_CITY_ESCAPED_RHYHORN  ; F1
	const TOGGLE_FUCHSIA_CITY_CHASING_WARDEN   ; F2

	toggle_consts_for UNDERGROUND_PATH_WEST_EAST
	const TOGGLE_UNDERGROUND_PATH_ROCKET_DESERTER ; F3
	const TOGGLE_UNDERGROUND_PATH_DISCARDED_UNIFORM ; F4
